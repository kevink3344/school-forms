...
| `IF EXISTS (…) UPDATE … ELSE INSERT …` | `UPDATE …` + `INSERT … SELECT … WHERE NOT EXISTS …` | ⚠️ **Shipped bug — found by live-testing, not by the suite.** `libSQL` cannot parse `IF` and raises `SQL_PARSE_ERROR … near IF`. `updateSubmissionValues` used this shape, so **every staff-only save returned `500` under `DB_MODE=turso`** while `typecheck`, `build` and all tests stayed green. Note `IF NOT EXISTS` is *fine* (legal SQLite DDL) — only the intervening `ELSE`/conditional-batch form is fatal. `ON CONFLICT … DO UPDATE` is **not** an available substitute here: neither dialect declares a UNIQUE index on `submission_values(submission_id, field_id)`, and adding one is a live SQL Server migration. Guarded by the `TSQL_ONLY` list in `server/src/db/driver/libsql.test.ts`. |
...
| 2026-09-15 | **rev. 5 — the conditional-batch class.** §5.1 gained a row for `IF EXISTS (…) UPDATE … ELSE INSERT …` → `UPDATE` + `INSERT … SELECT … WHERE NOT EXISTS`. A third instance of the same failure mode as `SELECT TOP 1`: a construct the plan's own §8.5 rule says to hand-build, that was left as T-SQL in `updateSubmissionValues`. It made **every** staff-only value save return `500` under Turso while typecheck, build and the full suite stayed green — a second confirmation that only a cross-backend parity harness or a live click finds this class. Also recorded that `ON CONFLICT … DO UPDATE` is *not* available as the substitute, because neither dialect declares the UNIQUE index it requires. Regression-guarded by the `TSQL_ONLY` list in `server/src/db/driver/libsql.test.ts`. |
| 2026-09-16 | **rev. 6 — a new table, and the one place `ON CONFLICT` *is* the right answer.** The per-user grid-column store (`user_form_view_columns`, §5.3 / `docs/plans/view-designer.md` §9) is the first table added since cut-over, so it lands on **both** dialects: 11 tables → **12**, and `DDL_STATEMENTS` 39 → **42** (create table, unique index, backfill). Three notes worth keeping: **(a)** unlike `submission_values` in rev. 5, this table *declares* its unique constraint — `UX_ufvc_user_form` on `(user_id, form_id)` — so `INSERT … ON CONFLICT(user_id, form_id) DO UPDATE … RETURNING id` is legitimate here and is what the Turso dialect uses. The rev. 5 finding was that the constraint was missing, not that the construct is unusable; declaring the constraint is the cheaper of the two fixes and is the right default for any new upsert. **(b)** SQL Server gets a `MERGE`, taking the `MERGE …` count from **2 to 3** — and it is the first one *outside* `queries.ts`, living in `dialect/sqlserver.ts` behind the `Dialect.upsertUserFormViewColumns()` method. New divergent DML belongs behind the dialect interface, not in a query body. **(c)** Adding a table to `schema.ts` is not enough for Turso: it also needs the `TURSO_DDL` entry, the `AddColumn`/index handling if it had been a column, **and** an entry in the `TABLES` list in `db/migrate-turso.ts` — otherwise a fresh turso database boots without it. The backfill (`forms.view_columns` → the designer's row) is guarded by `IF NOT EXISTS` / `IF OBJECT_ID(…,'U') IS NULL`, both of which stay legal under the `TSQL_ONLY` scan and make the whole batch idempotent, so it is safe to re-run on every boot. |
